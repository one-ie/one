 ...
 *
 * Ported from one.ie/web's StripeProvider + StripeCheckoutForm (the same
 * components the ONE shop plugin's `provider: 'stripe'` checkout uses).
 * Mounts a live PaymentElement against a PaymentIntent minted by this
 * site's own /api/pay/create-intent. Rendered only when the page confirms
 * both Stripe keys are configured — otherwise /payments shows the honest
 * CardCheckoutPreview instead.
 *
 * Inline-style chrome (Tailwind classNames don't compile in React islands
 * here — see PaymentLinkGenerator.tsx); the PaymentElement itself is styled
 * through Stripe's appearance API, mapped to the site's design tokens.
 */
