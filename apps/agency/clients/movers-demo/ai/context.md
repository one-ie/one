# Company context — Summit Movers

> **DEMO NODE — fictional sample data.** Summit Movers, its owner, phone, and the
> USDOT/MC/TxDMV numbers below are invented for demonstration. **Do not publish or point a
> real domain at this node.** It's a fully-filled worked example of the `mover` template
> (`../mover/`) — clone *that* for a real operator. Every agent reads this file on **every
> turn** as ground truth; for a real node, keep it true (agents inherit its errors).

## Who we are

- **Summit Movers** — a licensed, owner-operated moving company serving **Austin, TX**
  and surrounding areas since **2016**.
- We do **local moves, long-distance moves, packing, and storage** for households and small
  businesses. (Full list: `data/services.md`.)
- **Owner:** Marcus Hill · **Phone:** (512) 555-0142 (a human answers in business hours).
- **License:** USDOT 3987654 · MC 1456789 (interstate) · TxDMV 006789 (intrastate).
...
### Analytics (cross-department measurement)
Not a lifecycle — a lens over all four. Reads the D1 signal log and produces one Monday-morning
brief: pipeline velocity, response time, lead→estimate and estimate→booked rates, CSAT, review
and referral rates, CAC, and source attribution. Flags any KPI below its floor with one suggested
action. Targets + floors live in `data/analytics/kpis.toml`; the unified funnel view (conversion
targets + leak points) lives in `data/funnels/acquisition.toml`.
**Agent:** `mover--analytics` — weekly brief + KPI flags. Reports facts, one action per flag.

## Workflows that connect the departments

**Core funnel** (one contact, start to compounding asset):
...
**Growth + recovery** (fill the funnel, recover the leaks):

| Workflow | Trigger | What it does |
|---|---|---|
| `re-nurture` | `lifecycle:sales:lost` (+30d) | One proof-stack message + one follow-up to recover a stalled/lost lead. Never a drip. |
| `campaign-seasonal` | `manual` (6–8 wk pre-peak) | Summer/end-of-month campaign: infrastructure → awareness → proof → conversion. Education first, pitch last. |
| `b2b-partnerships` | `manual` (quarterly/city) | Reciprocal-framed outreach to apartment managers + RE agents for warm-intro lead flow. |

## Trust signals we lead with (lead-gen conversion levers)

- **USDOT 3987654 / MC 1456789** in footer, clickable to FMCSA SAFER.
- **TxDMV 006789** for intrastate moves where required.
...
- **Real Google reviews** (live embed, named-crew mentions), **named owner photo**, **real
  fleet + crew photos** (never stock), **"4,000+ moves since 2016"**.
