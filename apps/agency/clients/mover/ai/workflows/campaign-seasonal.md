...
description: 6–8 week pre-peak campaign for {{Company}}'s busy season (May–August peak; end-of-month surges). Builds awareness, educates the audience, then converts at peak. Infrastructure (sequences, city pages, GMB posts, ad creative) must be live before the campaign fires.
trigger: manual
...
> **Worked instance:** `data/campaigns/summer-2026/` (when built for this operator) is this
> workflow fully filled — manifest + real copy for every channel (hooks, Meta + Google ads,
> email sequence, GMB posts, city page). Read that pack to see the concrete output; this file
> is the process. See `../movers-demo/ai/workflows/campaign-seasonal.md` for a complete
> demo instance (Summit Moving Co., Austin TX).

**Goal:** Fill {{Company}}'s calendar for the peak moving season (May–August, end-of-month
...
args:
  ...
  prompt: |
    ...
    City pages
    [ ] All target cities have a live service-area page
    [ ] Each page has: H1 with city + service, trust block (license number/reviews/named owner/founded),
    ...
    [ ] At least 3 creative angles: trust (license), proof (review count), price clarity (estimate type)

    Email sequences
    [ ] 3-email awareness sequence staged in CRM (cost guide → reviews → date-availability)
    [ ] lead-capture workflow confirmed live

    GMB
    [ ] 4 GMB posts scheduled for the 8-week run (1 per 2 weeks)
    [ ] Google Business Profile Q&A answered: binding estimate?, license number?, deposit required?
...
`{{company_slug}}--marketing` agent produces one piece per week:

| Week | Format | Topic |
|---|---|---|
| –6 | Cost guide blog + GMB post | "How much does a local move cost in {{Metro}} in {{year}}?" Real price ranges |
| –5 | City spotlight page | "[City] apartment movers — what to expect, what to ask" |
| –4 | "How to vet a mover" checklist | License check, binding vs non-binding, deposit red flags |
...
args:
  ...
  content: |
    ...
    Week –2: "{{ review_count }}+ moves since {{ founded }}. Here's what we've learned about
             [city] traffic / buildings / timing." Local knowledge signals = trust.
```
...
Message 3 (day –7):
  ...
  Our rate for a {{ home_size }} local move in {{ city }}: roughly {{ price_range }}.
...
Message 4 (day –3):
  ...
  If that's your window: {{ estimate_link }} or call {{Phone}}.
```
...
- **Pre-launch = education, not pitch.** Pre-launch must be education / inspiration / curiosity.
  Pitching too early kills warm leads.
...
- Fill `{{Phone}}`, `{{Company}}`, `{{Metro}}`, `{{Founded}}` from `_node.toml` before `one push`.
