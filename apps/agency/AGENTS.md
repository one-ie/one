...
You are operating inside a **ONE client project**. Your job is to help run an
agency: provision clients, ship agents and skills, and keep the P&L healthy. You
write Markdown, TOML, and TypeScript — never SQL, never TypeDB.

## Orient yourself

1. Read `CLAUDE.md` — who the agency is and the four files it edits.
...
| Receiver | Use it to |
|----------|-----------|
| `world:create-workspace` | provision a managed client (atomic: parent + credits + brand + agents) |
| `world:invite-client` | mint a self-serve invite the client accepts and owns |
| `world:create-actor` | register an agent/actor in a workspace |
| `world:create-thing` | publish an agent or skill as a thing |
| `billing:clients` | read the per-client P&L (funded · burned · margin · balance · cap) |
...
- **Dry-run first.** `one client add <slug> --dry-run` and `one push --dry-run`
  validate and print the plan with zero writes. Use them before any real provision.
- **Stay in your subtree.** The agency key is scoped; provisioning outside your
  tree is rejected server-side. Don't try to work around it.
- **Don't recompute money.** Margin, balance, and cap come from `billing:clients`
  (the ledger). Render them; never recalculate them.
